...
import database
from common import config, event_listener, format_datetime, hybrid_check, limit_len, mention_datetime, select_view
from features import sugestie
from features.utils import check_staff

class NoRulesError(app_commands.CheckFailure):
  pass
...
rules = app_commands.Group(name='rules', description='Komendy do regulaminu', guild_ids=[config['guild']])
...
@app_commands.rename(ile_sugestii='ile-sugestii')
@check_staff('ustanawiania nowego regulaminu')
async def set_(interaction, text: discord.Attachment, ile_sugestii: app_commands.Range[int, 0, 4]): # Max is 4 due to a limitation in Discord.
  try:
