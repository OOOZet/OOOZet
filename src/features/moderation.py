...
@app_commands.context_menu(name='Skickuj')
@app_commands.guild_only
@check_staff('kickowania')
...
@app_commands.context_menu(name='Zbanuj')
@app_commands.guild_only
@check_staff('banowania')
...
@app_commands.command(name='unban', description='Odbanowuje użytkownika')
@app_commands.guild_only
@check_staff('odbanowywania')
async def cmd_unban(interaction, user: discord.User):
  await unban(interaction, user)

@app_commands.context_menu(name='Odbanuj')
@app_commands.guild_only
@check_staff('odbanowywania')
...
@app_commands.command(name='purge-everywhere', description='Usuwa wiadomości użytkownika ze wszystkich kanałów')
@app_commands.guild_only
...
@app_commands.context_menu(name='Usuń wiadomości wszędzie')
@app_commands.guild_only
@check_staff('usuwania wiadomości')
async def menu_purge_everywhere(interaction, user: discord.User):
  await purge_everywhere(interaction, user)

@app_commands.command(description='Usuwa wiadomości na tym kanale')
@app_commands.guild_only
@app_commands.rename(from_='from')
@app_commands.describe(from_='ID pierwszej wiadomości do usunięcia', to='ID ostatniej wiadomości do usunięcia')
@check_staff('usuwania wiadomości')
...
@app_commands.command(name='timeout-here', description='Timeoutuje autorów ostatnich wiadomości na tym kanale')
@app_commands.guild_only
@app_commands.rename(max_age='max-age')
@app_commands.describe(max_age='Maksymalny wiek rozważanych wiadomości')
@app_commands.choices(
  max_age=[app_commands.Choice(name=label, value=duration) for label, duration in config['timeout_here_max_age_choices']],
  duration=[app_commands.Choice(name=label, value=duration) for label, duration in [
    ['60 sekund', '60s'],
    ['5 minut', '5m'],
    ['10 minut', '10m'],
    ['1 godzina', '1h'],
