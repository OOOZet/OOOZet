...
from common import config, event_listener, hybrid_check, pages_view, parse_duration
...
class NoXpRolesError(app_commands.CheckFailure):
  pass
...
async def on_message(msg):
  member = msg.author
  if member.bot or msg.is_system() or msg.guild is None or msg.guild.id != config['guild'] or (msg.channel.id not in config['xp_unignored_channels'] and (msg.channel.id in config['xp_ignored_channels'] or msg.channel.category_id in config['xp_ignored_categories'])):
    return
...
xp = app_commands.Group(name='xp', description='Komendy do XP')
...
@app_commands.context_menu(name='Pokaż XP')
...
@console.operation(desc='transfers XP from one account to another')
async def transfer(from_: int, to: int):
  with lock:
...
@console.operation(desc='makes sure the XP database entry for a user exists')
def init(user: int):
  with lock:
