...
def setup(bot):
  ...
  async def show(interaction):
    ...
    async def callback(interaction2, choice):
      ...
      await interaction2.response.send_message(format_sugestia(sugestia), ephemeral=True)
    ...
    for sugestia in sugestie:
      if sugestia['did_pass'] is None:
        ...
        emoji = '✅'
      select.add_option(label=sugestia['text'], value=sugestia['id'], description=format_datetime(sugestia['vote_start']), emoji=emoji)
